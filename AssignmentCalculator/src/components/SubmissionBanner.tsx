...
const SubmissionBanner: React.FC<SubmissionResult> = ({showNotification, setNotification, successful=true}) => {
    ...
    return (
        <Transition appear show={showNotification}
            ...
            leaveTo="scale-y-0"
        >
            <div className={`text-white overflow-hidden flex items-center justify-center text-center fixed t-0 r-0 w-full h-10 ${successful?"bg-green-400":"bg-red-400"}`}>
                {/* Shows alternative messages and boxes based on the outcome of adding an assignment */}
                <span className="px-5">{successful?<CheckBadgeIcon className="w-7 h-7 text-white"/>:<ExclamationCircleIcon className="w-7 h-7 text-white"/>}</span>      
