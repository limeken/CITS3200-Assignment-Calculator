...
import type { Assignment, AssignmentCalendar} from "./CalendarTypes";
...
const PriorityQueue: React.FC<{
    ...
    ({newest, onUpdate, onDelete}) => {
        ...
        const deleteAssignment = (assignment: AssignmentCalendar) => {
        ...
        useEffect(() => {
            setSortedAssignments(insertionSort(sortedAssignments, newest))
        }, [newest])
        ...
        const pickPriority = (end: Date) => {
            ...
            if (current > end.getTime() - MED && current < end.getTime() - HIGH) {
                ...
                return {base: "bg-green-200", days: differenceInDays(end, Date.now())}
            }
        }

        const openAssigment = (assignment: Assignment) => {
        open((id: string) => (
               <AssignmentModal
